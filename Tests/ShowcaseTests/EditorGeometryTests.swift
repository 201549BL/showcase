...
struct EditorGeometryTests {
    @Test("Timeline ruler uses readable intervals across recording lengths")
    func timelineRulerIntervals() {
        let short = TimelineRuler(duration: 10.9, width: 900)
        #expect(short.step == 2)
        #expect(short.ticks == [0, 2, 4, 6, 8, 10])
        let subsecond = TimelineRuler(duration: 0.6, width: 900)
        #expect(subsecond.label(for: 0.2) == "00:00.2")
        #expect(Set(subsecond.ticks.map { subsecond.label(for: $0) }).count == subsecond.ticks.count)
        let long = TimelineRuler(duration: 3_600, width: 400)
        #expect(long.ticks.count <= 5)
        #expect(long.ticks.allSatisfy { $0 <= 3_600 })
        #expect(TimelineRuler(duration: 0, width: 0).ticks == [0])
        #expect(TimelineRuler(duration: .infinity, width: 900).ticks == [0])
    }
