...
struct CaptureSourceService {
    ...
    func thumbnail(for descriptor: CaptureSourceDescriptor) async throws -> NSImage {
        let source = try await resolve(descriptor)
        let configuration = SCStreamConfiguration()
        let ratio = descriptor.frame.width / max(1, descriptor.frame.height)
        configuration.width = 640
        configuration.height = max(2, Int(640 / ratio))
        configuration.showsCursor = false
        configuration.capturesAudio = false
        let image = try await SCScreenshotManager.captureImage(
            contentFilter: source.contentFilter, configuration: configuration
        )
