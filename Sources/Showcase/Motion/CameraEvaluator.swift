...
/// `state(at:)` calls. Pan and zoom share a bounded pose but can use separate
/// response times; every retarget inherits the existing velocity.
...
private struct CameraTrajectory {
    private struct Configuration {
        ...
        func proportionalPanDuration(for scale: Double) -> Double {
            // The cursor path is already smoothed. A shorter camera response
            // avoids stacking a full follow delay on top of that filtering.
            cursorFollowDuration(for: scale) * 0.5
        }
    ...
    private struct Target {
        ...
        var exitProgress: Double? = nil
    }

    /// Map the inner 80% of the recording onto the full pan range, so controls
    /// near an edge reveal that edge without requiring the cursor to touch it.
    /// SpringPose projects pan into the bounds of the *rendered* zoom level,
    /// including during entrance and exit.
    private struct ProportionalCursorPan {
        private static let edgeMargin = 0.1

        var segmentID: UUID?
        var horizontalPan = 0.0
        var verticalPan = 0.0
        var hasObservedCursor = false
        ...
        mutating func pose(
            ...
            if segmentID != segment.id {
                ...
                hasObservedCursor = false
            }
            if let cursor = cursorPath?.frame(at: time), cursor.opacity > 0 {
                let x = Self.panFraction(for: Double(cursor.position.x / sourceSize.width))
                let y = Self.panFraction(for: Double(cursor.position.y / sourceSize.height))
                hasObservedCursor = true
                horizontalPan = asin(1 - 2 * x)
                verticalPan = asin(1 - 2 * y)
            }
            guard hasObservedCursor else {
                return SpringPose(cameraState: target.state, sourceSize: sourceSize)
            }
            return SpringPose(
                horizontalPan: horizontalPan,
                verticalPan: verticalPan,
                logScale: log(max(1, target.state.scale))
            )
        }
        private static func panFraction(for cursorFraction: Double) -> Double {
            ((cursorFraction - edgeMargin) / (1 - 2 * edgeMargin)).clamped(to: 0...1)
        }
    }
    ...
    init(
        ...
        let sortedSegments = segments.map { segment in
            var normalized = segment
            normalized.normalizeTiming()
            return normalized
        }.sorted {
        ...
        for sampleIndex in 1...sampleCount {
            ...
            if let segment = baseTarget.cursorVisibilitySegment, segment.usesProportionalCursorPan {
                cursorFollower.reset()
                cursorVisibilityGuard = CursorVisibilityGuard()
                let pose = proportionalPan.pose(
                    for: baseTarget,
                    segment: segment,
                    cursorPath: cursorPath,
                    at: time,
                ...
                // Cursor motion changes pan response only; zoom timing continues
                // to come from the segment's authored entrance/exit durations.
                spring.advance(
                    toward: pose,
                    omega: configuration.springSettlingConstant / max(
                        sampleInterval, baseTarget.responseDuration
                    ),
                    panOmega: proportionalPan.hasObservedCursor
                        ? configuration.springSettlingConstant
                            / configuration.proportionalPanDuration(for: baseTarget.state.scale)
                        : nil,
                    deltaTime: sampleInterval
                )
                // Only a hard visibility rail remains for rapid cursor jumps.
                // No travel band, look-ahead, or generated reframe steers auto pan.
                if let cursor = cursorPath?.frame(at: time), cursor.opacity > 0 {
                    let state = spring.value.cameraState(sourceSize: sourceSize)
                    let visible = Self.keepingVisible(
                        cursor.position,
                        in: state,
                        sourceSize: sourceSize,
                        viewportFraction: 1,
                    ...
                    if visible.focusPoint.distance(to: state.focusPoint) > 0.000_001 {
                        spring.constrainPan(
                            to: visible.focusPoint,
                            sourceSize: sourceSize,
                            previousPose: nil,
                            deltaTime: sampleInterval,
                            matchHorizontalVelocity: false,
                            matchVerticalVelocity: false
                        )
                    }
                }
            } else {
                proportionalPan = ProportionalCursorPan()
                ...
                spring.advance(
                    ...
                    omega: configuration.springSettlingConstant / responseDuration,
                    deltaTime: sampleInterval
                )
            ...
            if let progress = baseTarget.exitProgress, let segment = baseTarget.cursorVisibilitySegment {
                if exitAnchor?.id != segment.id {
                    exitAnchor = (segment.id, spring.value.logScale)
                }
                // A spring approaches its target asymptotically. Bound its final
                // tail with a smooth envelope so the visible exit finishes at the
                // block edge, including very short clips, without a hard snap.
                let p = progress.clamped(to: 0...1)
                let eased = p * p * p * (p * (p * 6 - 15) + 10)
                let limit = (exitAnchor?.logScale ?? 0) * (1 - eased)
                if spring.value.logScale > limit {
                    spring.value.logScale = limit
                    spring.velocity.logScale = min(0, spring.velocity.logScale)
                }
                if p == 1 { spring.velocity.logScale = 0 }
            } else {
                exitAnchor = nil
            }
            builtSamples.append(spring.value)
    ...
    private static func target(
        ...
        if time < timing.exitStart {
            let activeReframe = current.reframes.filter { $0.time <= time }.max { $0.time < $1.time }
            ...
            return Target(
                ...
                } ?? timing.entranceDuration,
                segment: current,
                cursorVisibilitySegment: current,
                usesDeliberateReframe: !current.usesProportionalCursorPan,
                authoredReframeID: activeReframe?.id
            )
        }

        if time <= current.endTime + timing.exitDuration {
            return Target(
                state: overview.state,
                responseDuration: timing.exitDuration,
                segment: nil,
                cursorVisibilitySegment: current,
                exitProgress: (time - timing.exitStart) / max(0.000_001, timing.exitDuration)
            )
        }
